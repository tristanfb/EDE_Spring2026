...
```{r project.setup, echo=TRUE, results='hide', message=FALSE, error=FALSE, warning=FALSE}
#Import libraries
library(tidyverse)
library(here)
library(knitr)
...
knitr::kable(summary_PM25, digits=2, caption = "Mean Particulates (2.5mm)", col.names = c("County", "$\\mu g/m^3$")) 

knitr::kable(summary_Ozone, digits = 2, caption = "Mean Ozone", col.names = c("County", "$\\mu g/m^3$")) 
...
```{r boxplot 2018, echo=FALSE, fig.cap =" Monthly Ozone 2018", message=FALSE, error=FALSE, warning=FALSE, fig.align="left", fig.height=3.5,}

epa_data$Month <- factor(epa_data$Month,
                   levels = 1:12,
                   labels = month.abb)

ggplot(subset(epa_data, Year==2018)) + 
  geom_boxplot(aes(x=Month, y=Ozone)) +
  labs(x = NULL,
       y = expression(Ozone~(mu*g/m^3)),
       title = "Monthly Ozone: 2018")


```

```{r boxplot 2019, echo=2019, fig.cap =" Monthly Ozone 2019", message=FALSE, error=FALSE, warning=FALSE, fig.align="left", fig.height=3.5,}
ggplot(subset(epa_data, Year==2019)) + 
  geom_boxplot(aes(x=Month, y=Ozone)) +
  labs(x = NULL,
       y = expression(Ozone~(mu*g/m^3)),
       title = "Monthly Ozone: 2019")
...
## Git Repository
https://github.com/tristanfb/EDE_Spring2026

I did not use AI in the completion of this assignment - Tristan Frappier-Brinton, March 2, 2026